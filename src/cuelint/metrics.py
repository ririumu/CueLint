...
def calculate_metrics(document: NormalizedDocument, evidence: list[EvidenceRow]) -> SummaryMetrics:
    return SummaryMetrics(
        counts_by_family=count_by_family(evidence),
        response_length_chars=len(document.original_text),
        paragraph_count=len(document.paragraphs),
        sentence_count=len(document.sentences),
        token_count=document.token_count,
        evidence_count=len(evidence),
        cue_density_per_100_tokens=calculate_cue_density(len(evidence), document.token_count),
...
def count_by_family(evidence: list[EvidenceRow]) -> dict[str, int]:
    return dict(sorted(Counter(row.family for row in evidence).items()))


def calculate_cue_density(evidence_count: int, token_count: int) -> float:
    if token_count <= 0:
        return 0.0
    return round((evidence_count / token_count) * 100, 4)
